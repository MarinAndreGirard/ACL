...
def create_H_new(d1,d2, E_s, E_s2, E_int_s, E_int_e,E_int2,E_e, E_e2):
    ...
    Returns:
        ...
        The return as a function of parameters is: H = H_s + H_int + H_e = H_s_self x id + H_int_s x H_int_e + id x H_e_self = [(E_s x diag(i) + E_s2 x id)x id ] + [(E_int_s x off_diag_sqrt(i)) x (E_int_e x rd_mat1)+(E_int2 x id x id)] + [id x (E_e x H_e_self + E_e2 x id)]
    """
    ...
    #Making H_int
    H_int = qt.tensor(H_int_s, H_int_e) + E_int2*qt.qeye(d) # Extend to full Hilbert space + add constant scale factor
